def make_api_request(url, headers, params=None, timeout=GITHUB_REQUEST_TIMEOUT):
...
def fetch_paginated_data(url, headers, params=None, per_page=GITHUB_PER_PAGE):
    ...
    while current_url:
        response = make_api_request(current_url, headers=headers, params=params if '?' not in current_url else None, timeout=GITHUB_REQUEST_TIMEOUT)
     ...
     if response and response.status_code == 200:
         try:
             ...
             if isinstance(content_data, dict) and content_data.get('type') == 'file' and 'content' in content_data:
                 ...
                 print(f"        Warning: Could not get file content for {file_path} @ {commit_sha[:7]}. Unexpected format.")
                 return ""
         except json.JSONDecodeError as e:
         ...
         print(f"        Warning: Failed to fetch file content for {file_path} @ {commit_sha[:7]}. Status: {response.status_code if response else 'N/A'}")
         return ""
...
def github_process_commit_files_list(owner, repo, commit_sha, headers):
    ...
    try:
        ...
        print(f"    Error decoding JSON response for commit details {commit_sha[:7]}: {e}. Skipping file list processing.")
        return [], 0, 0, 0
    except Exception as e:
        print(f"    Unexpected error processing commit files list for {commit_sha[:7]}: {e}. Skipping file list processing.")
        return [], 0, 0, 0
...
def github_process_pr_files(owner, repo, pr_number, base_sha, head_sha, headers, pr_output_dir):
    ...
    files_list = fetch_paginated_data(api_url, headers=headers, per_page=GITHUB_PER_PAGE)
    if not files_list:
...
def github_get_commit_check_runs(owner, repo, ref_sha, headers):
    ...
    response = make_api_request(api_url, headers=headers, params={'per_page': GITHUB_PER_PAGE}, timeout=GITHUB_REQUEST_TIMEOUT)
    ...
    for commit_data in all_commits:
        ...
        if commit and isinstance(commit, dict) and isinstance(parents, list) and len(parents) > 1:
            merge_commit_info = {
                ...
                'api_committer_login': commit_data.get('committer').get('login') if commit_data.get('committer') else None,
...
def fetch_github_data(owner, repo, pr_state='all', branch_for_merge_history='master', merge_history_since=None, merge_history_until=None):
    """
    ...
    print(f"\n--- Generating Summary of Changes to {SUMMARY_CSV_FILE} ---")
    all_changes_summary = []
    ...
    for pr_data in processed_prs_metadata:
        ...
        all_changes_summary.append([pr_author_login, pr_author_name, pr_author_email, change_type, change_id, change_date, directory])
    ...
    for mc_data in merge_commits_history_list:
        ...
        # Add email to the list
        all_changes_summary.append([mc_author_login, mc_author_name, mc_author_email, change_type, change_id, change_date, directory])
    ...
    try:
        with open(summary_filepath, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            csv_writer.writerow(['Author Login', 'Author Name', 'Author Email', 'What (Type)', 'ID (PR#/Commit SHA)', 'When (Date)', 'Directory'])
...
class CodeChangeRAG:
    ...
    def _process_single_change(self, change_dir_name):
        ...
        try:
            ...
            return None
             ...
             print(f"An unexpected error occurred loading metadata for {change_type_prefix.upper()} {change_id}: {e}. Skipping.")
             return None

        if self.embeddings is None or self.splitter is None:
             print(f"Skipping vector DB creation for {change_type.upper()} {full_change_id}: Embeddings model or splitter not initialized.")
             return None
        ...
        for file_meta in tqdm(changed_files, desc=f"Processing files for {change_type.upper()} {full_change_id}"):
            if not isinstance(file_meta, dict) or 'filename' not in file_meta:
                 print(f"Warning: Skipping invalid file metadata entry for {change_type.upper()} {full_change_id}: {file_meta}")
                 continue
        ...
        if change_type == 'pr':
            ...
            for comment in issue_comments:
                 if isinstance(comment, dict) and comment.get("body"):
                      comment_chunks = self.splitter.split_text(f"Pull Request Issue Comment by {comment.get('user', 'N/A')}:\n{comment.get('body')}")
                      chunks.extend(comment_chunks)

            review_comments = metadata.get("review_comments", [])
            for comment in review_comments:
                 if isinstance(comment, dict) and comment.get("body"):
                      comment_chunks = self.splitter.split_text(f"Pull Request Review Comment by {comment.get('user', 'N/A')} on {comment.get('path', 'N/A')}:\n{comment.get('body')}")
                      chunks.extend(comment_chunks)
        ...
        if not chunks:
            print(f"No processable content found for {change_type.upper()} {full_change_id}. Skipping vector DB creation.")
            return None
        ...
        try:
            ...
            ################################################################################################################################
            print(f"Successfully created Chroma DB for {change_type.upper()} {full_change_id} in temporary directory: {temp_dir}")
        except Exception as e:
    ...
    def _create_context(self, metadata, filename, before_code, after_code, patch):
        ...
        title = metadata.get('title', metadata.get('message', 'No Title/Message')) # Use title for PR, message for commit
        author_login = metadata.get('author_login', metadata.get('api_author_login', 'N/A')) # Use PR author or commit author login
    ...
    def _format_sources(self, docs):
        ...
        for doc in docs:
            source_info = {
                ...
                "content_snippet": str(doc.page_content)[:500] + "..." if doc and hasattr(doc, 'page_content') else "N/A",
    ...
    def analyze_coder_activity(self, coder_login, changes_list):
        ...
        if self.llm is None:
             ...
             return {
                 ...
                 "total_commits": sum(c.get('commits_count', 1) if c.get('request_type') == 'pr' else 1 for c in changes_list), # Count commits in PRs + individual merge commits
        ...
        for change_data in tqdm(changes_list, desc=f"Processing changes for {coder_login}"):
            ...
            full_path = os.path.join(self.data_path, change_dir_name)
            if not os.path.isdir(full_path):
                 print(f"Warning: Data directory not found for {change_type.upper()} {change_id} at {full_path}. Skipping.")
                 continue
            ...
            try:
                ...
                if change_type == 'pr':
                    pr_body = metadata.get("body")
                    if pr_body:
                         all_coder_chunks.extend(self.splitter.split_text(f"Pull Request Body by {metadata.get('author_login', 'N/A')}:\n{pr_body}"))

                    issue_comments = metadata.get("issue_comments", [])
                    for comment in issue_comments:
                         if isinstance(comment, dict) and comment.get("body"):
                              all_coder_chunks.extend(self.splitter.split_text(f"Issue Comment by {comment.get('user', 'N/A')}:\n{comment.get('body')}"))

                    review_comments = metadata.get("review_comments", [])
                    for comment in review_comments:
                         if isinstance(comment, dict) and comment.get("body"):
                              all_coder_chunks.extend(self.splitter.split_text(f"Review Comment by {comment.get('user', 'N/A')} on {comment.get('path', 'N/A')}:\n{comment.get('body')}"))
                elif change_type == 'merge_commit':
                     commit_message = metadata.get("message")
                     if commit_message:
                          all_coder_chunks.extend(self.splitter.split_text(f"Merge Commit Message by {metadata.get('api_author_login', 'N/A')}:\n{commit_message}"))
                     linked_issues = metadata.get("linked_issues_parsed", [])
                     if linked_issues:
                          issues_text = "Linked Issues: " + ", ".join(linked_issues)
